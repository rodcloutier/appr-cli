...
def test_inspect_init(cli_parser):
    inspectcmd = get_inspectcmd(cli_parser, ["foo/bar", "-t", "helm"])
    assert inspectcmd.version == "default"
    assert inspectcmd.registry_host is None
...
def test_inspect_tree(cli_parser, package_blob, capsys):
    ...
    with requests_mock.mock() as m:
        ...
        m.get("https://kpm.sh/api/v1/packages/foo/bar/1.0.0/helm/pull", content=response)
        inspectcmd.exec_cmd()
